...
std::vector<Grid> STile::rotateWillOccupy() const {
	std::vector<Grid> res;
...
void STile::initGrids() {
	color = vec3(0.0f, 0.3f, 0.4f);
...
std::vector<Grid> STile::getGrids() const {
	return grids;
}

vec3 STile::getColor() const {
	return color;
}